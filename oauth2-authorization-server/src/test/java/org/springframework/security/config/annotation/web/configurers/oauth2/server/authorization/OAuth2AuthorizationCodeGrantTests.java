...
public class OAuth2AuthorizationCodeGrantTests {
  ...
  public void requestWhenAuthorizationRequestNotAuthenticatedThenUnauthorized() throws Exception {
    ...
    this.mvc
        .perform(
            get(DEFAULT_AUTHORIZATION_ENDPOINT_URI)
  ...
  private void assertAuthorizationRequestRedirectsToClient(String authorizationEndpointUri)
      throws Exception {
    RegisteredClient registeredClient = TestRegisteredClients.registeredClient().build();
    this.registeredClientRepository.save(registeredClient);
  ...
  public void requestWhenTokenRequestValidThenReturnAccessTokenResponse() throws Exception {
    ...
    OAuth2Authorization authorization =
        TestOAuth2Authorizations.authorization(registeredClient).build();
  ...
  @Test
  public void requestWhenTokenRequestCustomEndpointThenReturnAccessTokenResponse()
      throws Exception {
    this.spring.register(AuthorizationServerConfigurationCustomEndpoints.class).autowire();
  ...
  private OAuth2AccessTokenResponse assertTokenRequestReturnsAccessTokenResponse(
      RegisteredClient registeredClient, OAuth2Authorization authorization, String tokenEndpointUri)
      throws Exception {
    MvcResult mvcResult =
        this.mvc
            .perform(
    ...
    OAuth2Authorization accessTokenAuthorization =
  ...
  public void requestWhenPublicClientWithPkceThenReturnAccessTokenResponse() throws Exception {
    ...
    String authorizationCode = extractParameterFromRedirectUri(redirectedUrl, "code");
    OAuth2Authorization authorizationCodeAuthorization =
        this.authorizationService.findByToken(authorizationCode, AUTHORIZATION_CODE_TOKEN_TYPE);
    ...
    this.mvc
        .perform(
            post(DEFAULT_TOKEN_ENDPOINT_URI)
    ...
    OAuth2Authorization accessTokenAuthorization =
        this.authorizationService.findById(authorizationCodeAuthorization.getId());
    assertThat(accessTokenAuthorization).isNotNull();
    assertThat(accessTokenAuthorization.getAccessToken()).isNotNull();
  ...
  @Test
  public void requestWhenConfidentialClientWithPkceAndMissingCodeVerifierThenBadRequest()
      throws Exception {
    this.spring.register(AuthorizationServerConfiguration.class).autowire();
  ...
  public void requestWhenConsentRequestThenReturnAccessTokenResponse() throws Exception {
    ...
    OAuth2Authorization authorization =
        TestOAuth2Authorizations.authorization(registeredClient).principalName("user").build();
    this.authorizationService.save(authorization);

    MvcResult mvcResult =
        this.mvc
            .perform(
                post(DEFAULT_AUTHORIZATION_ENDPOINT_URI)
                    .param(OAuth2ParameterNames.CLIENT_ID, registeredClient.getClientId())
  ...
  public void requestWhenCustomConsentPageConfiguredThenRedirect() throws Exception {
    ...
    assertThat(redirectedUrl)
        .matches("http://localhost/oauth2/consent\\?scope=.+&client_id=.+&state=.+");
    ...
    assertThat(redirectQueryParams.getFirst(OAuth2ParameterNames.SCOPE))
        .isEqualTo("message.read message.write");
    ...
    OAuth2Authorization authorization =
        this.authorizationService.findByToken(state, STATE_TOKEN_TYPE);
    assertThat(authorization).isNotNull();
  }
    ...
    mvcResult =
        this.mvc
            .perform(
                post(DEFAULT_TOKEN_ENDPOINT_URI)
                    .params(
                        getTokenRequestParameters(registeredClient, authorizationCodeAuthorization))
                    .header(HttpHeaders.AUTHORIZATION, getAuthorizationHeader(registeredClient)))
            .andExpect(status().isOk())
            .andExpect(header().string(HttpHeaders.CACHE_CONTROL, containsString("no-store")))
            .andExpect(header().string(HttpHeaders.PRAGMA, containsString("no-cache")))
  ...
  public void requestWhenAuthorizationEndpointCustomizedThenUsed() throws Exception {
    this.spring
        .register(AuthorizationServerConfigurationCustomAuthorizationEndpoint.class)
        .autowire();
    ...
    OAuth2AuthorizationCodeRequestAuthenticationToken authorizationCodeRequestAuthenticationResult =
        OAuth2AuthorizationCodeRequestAuthenticationToken.with(
            ...
            .build();
    when(authorizationRequestConverter.convert(any()))
        .thenReturn(authorizationCodeRequestAuthenticationResult);
    when(authorizationRequestAuthenticationProvider.supports(
            eq(OAuth2AuthorizationCodeRequestAuthenticationToken.class)))
        .thenReturn(true);
    ...
    verify(authorizationResponseHandler)
        .onAuthenticationSuccess(any(), any(), eq(authorizationCodeRequestAuthenticationResult));
  }

  // gh-482
  @Test
  public void requestWhenClientObtainsAccessTokenThenClientAuthenticationNotPersisted()
      throws Exception {
    this.spring
        .register(AuthorizationServerConfigurationWithSecurityContextRepository.class)
        .autowire();
    ...
    ArgumentCaptor<org.springframework.security.core.context.SecurityContext>
        securityContextCaptor =
            ArgumentCaptor.forClass(
                org.springframework.security.core.context.SecurityContext.class);
    verify(securityContextRepository, times(2))
        .saveContext(securityContextCaptor.capture(), any(), any());
    securityContextCaptor
        .getAllValues()
        .forEach(
            securityContext ->
    ...
    String authorizationCode =
        extractParameterFromRedirectUri(mvcResult.getResponse().getRedirectedUrl(), "code");
    OAuth2Authorization authorizationCodeAuthorization =
        this.authorizationService.findByToken(authorizationCode, AUTHORIZATION_CODE_TOKEN_TYPE);
  ...
  private static MultiValueMap<String, String> getAuthorizationRequestParameters(
      RegisteredClient registeredClient) {
    MultiValueMap<String, String> parameters = new LinkedMultiValueMap<>();
    parameters.set(
        OAuth2ParameterNames.RESPONSE_TYPE, OAuth2AuthorizationResponseType.CODE.getValue());
    parameters.set(OAuth2ParameterNames.CLIENT_ID, registeredClient.getClientId());
    parameters.set(
        OAuth2ParameterNames.REDIRECT_URI, registeredClient.getRedirectUris().iterator().next());
    parameters.set(
  ...
  private static MultiValueMap<String, String> getTokenRequestParameters(
  ...
  private static String scopeCheckbox(String scope) {
    return MessageFormat.format(
        "<input class=\"form-check-input\" type=\"checkbox\" name=\"scope\" value=\"{0}\""
            + " id=\"{0}\">",
        scope);
  }
  ...
  static class AuthorizationServerConfiguration {
    ...
    RegisteredClientRepository registeredClientRepository(
        JdbcOperations jdbcOperations, PasswordEncoder passwordEncoder) {
      ...
      jdbcRegisteredClientRepository.setRegisteredClientParametersMapper(
  ...
  @EnableWebSecurity
  @Import(OAuth2AuthorizationServerConfiguration.class)
  static class AuthorizationServerConfigurationWithTokenGenerator
      extends AuthorizationServerConfiguration {
  ...
  @EnableWebSecurity
  static class AuthorizationServerConfigurationCustomConsentPage
      extends AuthorizationServerConfiguration {
    // @formatter:off
    @Bean
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
      ...
      authorizationServerConfigurer.authorizationEndpoint(
          authorizationEndpoint -> authorizationEndpoint.consentPage(consentPage));
      RequestMatcher endpointsMatcher = authorizationServerConfigurer.getEndpointsMatcher();
  ...
  @EnableWebSecurity
  static class AuthorizationServerConfigurationCustomConsentRequest
      extends AuthorizationServerConfiguration {
    @Autowired private RegisteredClientRepository registeredClientRepository;
    ...
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
      ...
      OAuth2AuthorizationServerConfigurer<HttpSecurity> authorizationServerConfigurer =
          new OAuth2AuthorizationServerConfigurer<>();
      authorizationServerConfigurer.authorizationEndpoint(
          authorizationEndpoint -> authorizationEndpoint.authenticationProvider(createProvider()));
      RequestMatcher endpointsMatcher = authorizationServerConfigurer.getEndpointsMatcher();
    ...
    private AuthenticationProvider createProvider() {
      ...
      authorizationCodeRequestAuthenticationProvider.setAuthorizationConsentCustomizer(
          new AuthorizationConsentCustomizer());
    ...
    static class AuthorizationConsentCustomizer
      ...
      public void accept(
        ...
        RegisteredClient registeredClient =
            authorizationConsentAuthenticationContext.getRegisteredClient();
        ClientSettings clientSettings = registeredClient.getClientSettings();

        Set<String> requestedAuthorities =
            authorities((String) additionalParameters.get("authority"));
        Set<String> allowedAuthorities =
            authorities(clientSettings.getSetting("custom.allowed-authorities"));
  ...
  @EnableWebSecurity
  static class AuthorizationServerConfigurationCustomAuthorizationEndpoint
      extends AuthorizationServerConfiguration {
    // @formatter:off
    @Bean
    public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http)
