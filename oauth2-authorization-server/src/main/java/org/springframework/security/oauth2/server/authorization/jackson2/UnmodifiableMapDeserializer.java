 ...
 *
 * <p>A {@code JsonDeserializer} for {@link Collections#unmodifiableMap(Map)}.
 *
 * @author Joe Grandja
 * @since 5.3
...
final class UnmodifiableMapDeserializer extends JsonDeserializer<Map<?, ?>> {
  ...
  @Override
  public Map<?, ?> deserialize(JsonParser parser, DeserializationContext context)
      throws IOException {
    ...
    if (mapNode != null && mapNode.isObject()) {
      ...
      for (Map.Entry<String, JsonNode> field : fields) {
        result.put(
            field.getKey(), mapper.readValue(field.getValue().traverse(mapper), Object.class));
      }
    }
