...
public final class TokenSettings extends AbstractSettings {
  ...
  /**
   * Returns the token format for an access token. The default is {@link
   ...
   * {@code false} if a new refresh token is issued. The default is {@code true}.
   */
  ...
  /**
   * Returns the {@link SignatureAlgorithm JWS} algorithm for signing the {@link OidcIdToken ID
   * Token}. The default is {@link SignatureAlgorithm#RS256 RS256}.
   *
   * @return the {@link SignatureAlgorithm JWS} algorithm for signing the {@link OidcIdToken ID
   *     Token}
   */
  ...
  public static class Builder extends AbstractBuilder<TokenSettings, Builder> {
    ...
    public Builder accessTokenTimeToLive(Duration accessTokenTimeToLive) {
      ...
      Assert.isTrue(
          accessTokenTimeToLive.getSeconds() > 0,
          "accessTokenTimeToLive must be greater than Duration.ZERO");
      return setting(
          ConfigurationSettingNames.Token.ACCESS_TOKEN_TIME_TO_LIVE, accessTokenTimeToLive);
    }
     ...
     *
     * @param reuseRefreshTokens {@code true} to reuse refresh tokens, {@code false} to issue new
     *     refresh tokens
     * @return the {@link Builder} for further configuration
     */
    ...
    public Builder refreshTokenTimeToLive(Duration refreshTokenTimeToLive) {
      ...
      Assert.isTrue(
          refreshTokenTimeToLive.getSeconds() > 0,
          "refreshTokenTimeToLive must be greater than Duration.ZERO");
      return setting(
          ConfigurationSettingNames.Token.REFRESH_TOKEN_TIME_TO_LIVE, refreshTokenTimeToLive);
    }

    /**
     * Sets the {@link SignatureAlgorithm JWS} algorithm for signing the {@link OidcIdToken ID
     * Token}.
     *
    ...
    public Builder idTokenSignatureAlgorithm(SignatureAlgorithm idTokenSignatureAlgorithm) {
      ...
      return setting(
          ConfigurationSettingNames.Token.ID_TOKEN_SIGNATURE_ALGORITHM, idTokenSignatureAlgorithm);
    }
