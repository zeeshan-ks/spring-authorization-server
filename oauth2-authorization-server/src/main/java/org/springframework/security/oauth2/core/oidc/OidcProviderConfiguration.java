...
/**
 * A representation of an OpenID Provider Configuration Response, which is returned from an Issuer's
 * Discovery Endpoint, and contains a set of claims about the OpenID Provider's configuration. The
 * claims are defined by the OpenID Connect Discovery 1.0 specification.
 *
...
public final class OidcProviderConfiguration extends AbstractOAuth2AuthorizationServerMetadata
  ...
  public static class Builder extends AbstractBuilder<OidcProviderConfiguration, Builder> {
    ...
    public Builder subjectTypes(Consumer<List<String>> subjectTypesConsumer) {
      acceptClaimValues(
          OidcProviderMetadataClaimNames.SUBJECT_TYPES_SUPPORTED, subjectTypesConsumer);
      return this;
    }

    /**
     * Add this {@link JwsAlgorithm JWS} signing algorithm to the collection of {@code
     * id_token_signing_alg_values_supported} in the resulting {@link OidcProviderConfiguration},
     * REQUIRED.
     *
     * @param signingAlgorithm the {@link JwsAlgorithm JWS} signing algorithm supported for the
     *     {@link OidcIdToken ID Token}
     * @return the {@link Builder} for further configuration
     */
    public Builder idTokenSigningAlgorithm(String signingAlgorithm) {
      addClaimToClaimList(
          OidcProviderMetadataClaimNames.ID_TOKEN_SIGNING_ALG_VALUES_SUPPORTED, signingAlgorithm);
      return this;
    }

    /**
     * A {@code Consumer} of the {@link JwsAlgorithm JWS} signing algorithms for the {@link
     * OidcIdToken ID Token} allowing the ability to add, replace, or remove.
     *
     * @param signingAlgorithmsConsumer a {@code Consumer} of the {@link JwsAlgorithm JWS} signing
     *     algorithms for the {@link OidcIdToken ID Token}
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * Use this {@code userinfo_endpoint} in the resulting {@link OidcProviderConfiguration},
     * OPTIONAL.
     *
     ...
     *
     * <p>The following claims are REQUIRED: {@code issuer}, {@code authorization_endpoint}, {@code
     * token_endpoint}, {@code jwks_uri}, {@code response_types_supported}, {@code
     * subject_types_supported} and {@code id_token_signing_alg_values_supported}.
     *
    ...
    protected void validate() {
      super.validate();
      Assert.notNull(
          getClaims().get(OidcProviderMetadataClaimNames.JWKS_URI), "jwksUri cannot be null");
      Assert.notNull(
          getClaims().get(OidcProviderMetadataClaimNames.SUBJECT_TYPES_SUPPORTED),
      ...
      Assert.notEmpty(
          (List<?>) getClaims().get(OidcProviderMetadataClaimNames.SUBJECT_TYPES_SUPPORTED),
      ...
      if (getClaims().get(OidcProviderMetadataClaimNames.USER_INFO_ENDPOINT) != null) {
        validateURL(
            getClaims().get(OidcProviderMetadataClaimNames.USER_INFO_ENDPOINT),
