 ...
 *
 * <p><b>NOTE:</b> This {@code RegisteredClientRepository} depends on the table definition described
 * in
...
public class JdbcRegisteredClientRepository implements RegisteredClientRepository {
  ...
  private static final String UPDATE_REGISTERED_CLIENT_SQL =
      "UPDATE "
          ...
          + " ?, redirect_uris = ?, scopes = ?, client_settings = ?, token_settings = ? WHERE "
          + PK_FILTER;
  // @formatter:on
  ...
  private void updateRegisteredClient(RegisteredClient registeredClient) {
    List<SqlParameterValue> parameters =
        new ArrayList<>(this.registeredClientParametersMapper.apply(registeredClient));
  ...
  /**
   * Sets the {@code Function} used for mapping {@link RegisteredClient} to a {@code List} of {@link
   * SqlParameterValue}. The default is {@link RegisteredClientParametersMapper}.
   *
   * @param registeredClientParametersMapper the {@code Function} used for mapping {@link
   *     RegisteredClient} to a {@code List} of {@link SqlParameterValue}
   */
  public final void setRegisteredClientParametersMapper(
    ...
    Assert.notNull(
        registeredClientParametersMapper, "registeredClientParametersMapper cannot be null");
  ...
  protected final Function<RegisteredClient, List<SqlParameterValue>>
  ...
  /**
   * The default {@code Function} that maps {@link RegisteredClient} to a {@code List} of {@link
   * SqlParameterValue}.
   */
    ...
    public List<SqlParameterValue> apply(RegisteredClient registeredClient) {
      Timestamp clientIdIssuedAt =
          registeredClient.getClientIdIssuedAt() != null
              ? Timestamp.from(registeredClient.getClientIdIssuedAt())
              : Timestamp.from(Instant.now());

      Timestamp clientSecretExpiresAt =
      ...
      return Arrays.asList(
          ...
          new SqlParameterValue(
              Types.VARCHAR, StringUtils.collectionToCommaDelimitedString(authorizationGrantTypes)),
          new SqlParameterValue(
              Types.VARCHAR,
          ...
          new SqlParameterValue(
              Types.VARCHAR, writeMap(registeredClient.getClientSettings().getSettings())),
          new SqlParameterValue(
              Types.VARCHAR, writeMap(registeredClient.getTokenSettings().getSettings())));
    }
