...
/**
 * A JDBC implementation of an {@link OAuth2AuthorizationConsentService} that uses a {@link
 * JdbcOperations} for {@link OAuth2AuthorizationConsent} persistence.
 *
 * <p><b>NOTE:</b> This {@code OAuth2AuthorizationConsentService} depends on the table definition
 * described in
...
public class JdbcOAuth2AuthorizationConsentService implements OAuth2AuthorizationConsentService {
  ...
  // @formatter:off
  private static final String UPDATE_AUTHORIZATION_CONSENT_SQL =
      "UPDATE " + TABLE_NAME + " SET authorities = ?" + " WHERE " + PK_FILTER;
  // @formatter:on
  ...
  private Function<OAuth2AuthorizationConsent, List<SqlParameterValue>>
  ...
  public JdbcOAuth2AuthorizationConsentService(
    ...
    this.authorizationConsentRowMapper =
        new OAuth2AuthorizationConsentRowMapper(registeredClientRepository);
  ...
  private void updateAuthorizationConsent(OAuth2AuthorizationConsent authorizationConsent) {
    List<SqlParameterValue> parameters =
        this.authorizationConsentParametersMapper.apply(authorizationConsent);
  ...
  public OAuth2AuthorizationConsent findById(String registeredClientId, String principalName) {
    ...
    List<OAuth2AuthorizationConsent> result =
        this.jdbcOperations.query(
            LOAD_AUTHORIZATION_CONSENT_SQL, pss, this.authorizationConsentRowMapper);
    return !result.isEmpty() ? result.get(0) : null;
  }

  /**
   * Sets the {@link RowMapper} used for mapping the current row in {@code java.sql.ResultSet} to
   * {@link OAuth2AuthorizationConsent}. The default is {@link OAuth2AuthorizationConsentRowMapper}.
   *
   * @param authorizationConsentRowMapper the {@link RowMapper} used for mapping the current row in
  ...
  /**
   * Sets the {@code Function} used for mapping {@link OAuth2AuthorizationConsent} to a {@code List}
   * of {@link SqlParameterValue}. The default is {@link
   * OAuth2AuthorizationConsentParametersMapper}.
   *
   * @param authorizationConsentParametersMapper the {@code Function} used for mapping {@link
   *     OAuth2AuthorizationConsent} to a {@code List} of {@link SqlParameterValue}
   */
  public final void setAuthorizationConsentParametersMapper(
      Function<OAuth2AuthorizationConsent, List<SqlParameterValue>>
          authorizationConsentParametersMapper) {
    Assert.notNull(
        authorizationConsentParametersMapper,
        "authorizationConsentParametersMapper cannot be null");
  ...
  protected final Function<OAuth2AuthorizationConsent, List<SqlParameterValue>>
      getAuthorizationConsentParametersMapper() {
    return this.authorizationConsentParametersMapper;
  }

  /**
   * The default {@link RowMapper} that maps the current row in {@code ResultSet} to {@link
  ...
  public static class OAuth2AuthorizationConsentRowMapper
    ...
    public OAuth2AuthorizationConsentRowMapper(
  ...
  /**
   * The default {@code Function} that maps {@link OAuth2AuthorizationConsent} to a {@code List} of
  ...
  public static class OAuth2AuthorizationConsentParametersMapper
    ...
    public List<SqlParameterValue> apply(OAuth2AuthorizationConsent authorizationConsent) {
      ...
      parameters.add(
          new SqlParameterValue(
              Types.VARCHAR, StringUtils.collectionToDelimitedString(authorities, ",")));
      return parameters;
    }
  }
}
