...
public final class NimbusJwsEncoder implements JwtEncoder {
  ...
  private JWK selectJwk(JoseHeader headers) {
    ...
    if (jwks.size() > 1) {
      throw new JwtEncodingException(
          String.format(
              ENCODING_ERROR_MESSAGE_TEMPLATE,
              "Found multiple JWK signing keys for algorithm '"
                  + headers.getAlgorithm().getName()
                  + "'"));
    }
  ...
  private static JoseHeader addKeyIdentifierHeadersIfNecessary(JoseHeader headers, JWK jwk) {
    ...
    if (StringUtils.hasText(headers.getKeyId())
        && StringUtils.hasText(headers.getX509SHA256Thumbprint())) {
      return headers;
    }
    ...
    if (!StringUtils.hasText(headers.getX509SHA256Thumbprint())
  ...
  private static class JwsHeaderConverter implements Converter<JoseHeader, JWSHeader> {
    ...
    public JWSHeader convert(JoseHeader headers) {
      JWSHeader.Builder builder =
          new JWSHeader.Builder(JWSAlgorithm.parse(headers.getAlgorithm().getName()));
