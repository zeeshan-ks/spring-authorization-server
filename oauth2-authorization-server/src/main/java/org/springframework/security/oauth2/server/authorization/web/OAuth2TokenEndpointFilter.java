...
/**
 * A {@code Filter} for the OAuth 2.0 Token endpoint, which handles the processing of an OAuth 2.0
 * Authorization Grant.
 *
 * <p>It converts the OAuth 2.0 Authorization Grant request to an {@link Authentication}, which is
 * then authenticated by the {@link AuthenticationManager}. If the authentication succeeds, the
 * {@link AuthenticationManager} returns an {@link OAuth2AccessTokenAuthenticationToken}, which is
 * returned in the OAuth 2.0 Access Token response. In case of any error, an {@link OAuth2Error} is
 * returned in the OAuth 2.0 Error response.
 *
 * <p>By default, this {@code Filter} responds to authorization grant requests at the {@code URI}
 * {@code /oauth2/token} and {@code HttpMethod} {@code POST}.
 *
 * <p>The default endpoint {@code URI} {@code /oauth2/token} may be overridden via the constructor
 * {@link #OAuth2TokenEndpointFilter(AuthenticationManager, String)}.
 *
 * @author Joe Grandja
 * @author Madhu Bhat
 ...
 * @see <a target="_blank" href="https://tools.ietf.org/html/rfc6749#section-3.2">Section 3.2 Token
 *     Endpoint</a>
 */
public final class OAuth2TokenEndpointFilter extends OncePerRequestFilter {
   ...
   * OAuth2AuthorizationGrantAuthenticationToken} used for authenticating the authorization grant.
   *
