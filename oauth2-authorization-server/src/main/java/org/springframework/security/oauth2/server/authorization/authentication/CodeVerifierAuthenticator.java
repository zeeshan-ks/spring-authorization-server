...
/**
 * An authenticator used for OAuth 2.0 Client Authentication, which authenticates the {@link
 * PkceParameterNames#CODE_VERIFIER code_verifier} parameter.
 *
...
final class CodeVerifierAuthenticator {
  ...
  private boolean authenticate(
    ...
    OAuth2Authorization authorization =
        this.authorizationService.findByToken(
            (String) parameters.get(OAuth2ParameterNames.CODE), AUTHORIZATION_CODE_TOKEN_TYPE);
  ...
  private static void throwInvalidGrant(String parameterName) {
    OAuth2Error error =
        new OAuth2Error(
            OAuth2ErrorCodes.INVALID_GRANT, "Client authentication failed: " + parameterName, null);
