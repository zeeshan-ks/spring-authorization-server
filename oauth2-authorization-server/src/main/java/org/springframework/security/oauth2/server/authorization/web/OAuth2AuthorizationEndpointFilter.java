...
/**
 * A {@code Filter} for the OAuth 2.0 Authorization Code Grant, which handles the processing of the
 * OAuth 2.0 Authorization Request (and Consent).
 *
 * @author Joe Grandja
 ...
 * @see <a target="_blank" href="https://datatracker.ietf.org/doc/html/rfc6749#section-4.1">Section
 ...
 * @see <a target="_blank"
 *     href="https://datatracker.ietf.org/doc/html/rfc6749#section-4.1.2">Section 4.1.2
...
public final class OAuth2AuthorizationEndpointFilter extends OncePerRequestFilter {
  ...
  private AuthenticationSuccessHandler authenticationSuccessHandler =
      this::sendAuthorizationResponse;
  ...
  public OAuth2AuthorizationEndpointFilter(
      AuthenticationManager authenticationManager, String authorizationEndpointUri) {
   ...
   *     extract an Authorization Request (or Consent) from {@link HttpServletRequest}
   */
   ...
   *     an {@link OAuth2AuthorizationCodeRequestAuthenticationException}
   */
  ...
  /**
   * Specify the URI to redirect Resource Owners to if consent is required. A default consent page
  ...
  private void sendAuthorizationConsent(
    ...
    Authentication principal =
        (Authentication) authorizationCodeRequestAuthenticationResult.getPrincipal();
    ...
    if (hasConsentUri()) {
      String redirectUri =
          UriComponentsBuilder.fromUriString(resolveConsentUri(request))
      ...
      DefaultConsentPage.displayConsent(
          request, response, clientId, principal, requestedScopes, authorizedScopes, state);
    }
  }
  ...
  private void sendAuthorizationResponse(
    ...
    if (StringUtils.hasText(authorizationCodeRequestAuthentication.getState())) {
      uriBuilder.queryParam(
          OAuth2ParameterNames.STATE, authorizationCodeRequestAuthentication.getState());
    }
  ...
  private static class DefaultConsentPage {
    ...
    private static void displayConsent(
        HttpServletRequest request,
        HttpServletResponse response,
        String clientId,
        Authentication principal,
      ...
      String consentPage =
          generateConsentPage(
              request, clientId, principal, requestedScopes, authorizedScopes, state);
    ...
    private static String generateConsentPage(
        HttpServletRequest request,
        String clientId,
        Authentication principal,
        Set<String> requestedScopes,
      ...
      builder.append(
          "            <p><span class=\"font-weight-bold text-primary\">"
              + clientId
      ...
      builder.append(
          "            <p>The following permissions are requested by the above app.<br/>Please"
              + " review these and consent if you approve.</p>");
      builder.append("        </div>");
      builder.append("    </div>");
      builder.append("    <div class=\"row\">");
      builder.append("        <div class=\"col text-center\">");
      builder.append(
          "            <form name=\"consent_form\" method=\"post\" action=\""
      ...
      for (String scope : scopesToAuthorize) {
        ...
        builder.append(
            "                    <input class=\"form-check-input\" type=\"checkbox\" name=\"scope\""
                + " value=\""
                + scope
                + "\" id=\""
                + scope
      ...
      if (!scopesPreviouslyAuthorized.isEmpty()) {
        builder.append(
            "                <p>You have already granted the following permissions to the above"
                + " app:</p>");
      ...
      builder.append(
          "                    <button class=\"btn btn-primary btn-lg\" type=\"submit\""
              + " id=\"submit-consent\">Submit Consent</button>");
      builder.append("                </div>");
      builder.append("                <div class=\"form-group\">");
      builder.append(
          "                    <button class=\"btn btn-link regular\" type=\"button\""
              + " onclick=\"cancelConsent();\" id=\"cancel-consent\">Cancel</button>");
      builder.append("                </div>");
      builder.append("            </form>");
      builder.append("        </div>");
      builder.append("    </div>");
      ...
      builder.append("        <div class=\"col text-center\">");
      builder.append(
          "            <p><small>Your consent to provide access is required.<br/>If you do not"
              + " approve, click Cancel, in which case no information will be shared with the"
              + " app.</small></p>");
      builder.append("        </div>");
      builder.append("    </div>");
      builder.append("</div>");
