...
/**
 * A {@code Filter} that processes OpenID Connect Dynamic Client Registration (and Configuration)
 * 1.0 Requests.
 *
 * @author Ovidiu Popa
 * @author Joe Grandja
...
public final class OidcClientRegistrationEndpointFilter extends OncePerRequestFilter {
   ...
   *
   * @param authenticationManager the authentication manager
   * @param clientRegistrationEndpointUri the endpoint {@code URI} for OpenID Client Registration
   *     requests
   */
  public OidcClientRegistrationEndpointFilter(
      AuthenticationManager authenticationManager, String clientRegistrationEndpointUri) {
    Assert.notNull(authenticationManager, "authenticationManager cannot be null");
    Assert.hasText(clientRegistrationEndpointUri, "clientRegistrationEndpointUri cannot be empty");
    this.authenticationManager = authenticationManager;
    this.clientRegistrationEndpointMatcher =
  ...
  protected void doFilterInternal(
    ...
    try {
      ...
      sendClientRegistrationResponse(
          response, httpStatus, clientRegistrationAuthenticationResult.getClientRegistration());
  ...
  private OidcClientRegistrationAuthenticationToken convert(HttpServletRequest request)
    ...
    if ("POST".equals(request.getMethod())) {
      OidcClientRegistration clientRegistration =
          this.clientRegistrationHttpMessageConverter.read(
