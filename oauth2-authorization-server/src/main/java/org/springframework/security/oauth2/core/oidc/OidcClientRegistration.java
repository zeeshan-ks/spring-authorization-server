...
/**
 * A representation of an OpenID Client Registration Request and Response, which is sent to and
 * returned from the Client Registration Endpoint, and contains a set of claims about the Client's
 * Registration information. The claims are defined by the OpenID Connect Dynamic Client
 ...
 * @see <a
 *     href="https://openid.net/specs/openid-connect-registration-1_0.html#RegistrationRequest">3.1.
 *     Client Registration Request</a>
 * @see <a
 *     href="https://openid.net/specs/openid-connect-registration-1_0.html#RegistrationResponse">3.2.
...
public final class OidcClientRegistration implements OidcClientMetadataClaimAccessor, Serializable {
  ...
  public static class Builder {
    ...
    /**
     * Sets the time at which the {@code client_secret} will expire or {@code null} if it will not
     * expire, REQUIRED if {@code client_secret} was issued.
     *
     * @param clientSecretExpiresAt the time at which the {@code client_secret} will expire or
     *     {@code null} if it will not expire
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * A {@code Consumer} of the redirection {@code URI} values used by the Client, allowing the
     * ability to add, replace, or remove, REQUIRED.
     *
     * @param redirectUrisConsumer a {@code Consumer} of the redirection {@code URI} values used by
     *     the Client
     * @return the {@link Builder} for further configuration
     */
     ...
     *
     * @param tokenEndpointAuthenticationMethod the authentication method used by the Client for the
     *     Token Endpoint
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * Sets the {@link JwsAlgorithm JWS} algorithm that must be used for signing the {@link Jwt JWT}
     * used to authenticate the Client at the Token Endpoint for the {@link
     * ClientAuthenticationMethod#PRIVATE_KEY_JWT private_key_jwt} and {@link
     * ClientAuthenticationMethod#CLIENT_SECRET_JWT client_secret_jwt} authentication methods,
     * OPTIONAL.
     *
     * @param authenticationSigningAlgorithm the {@link JwsAlgorithm JWS} algorithm that must be
     *     used for signing the {@link Jwt JWT} used to authenticate the Client at the Token
     *     Endpoint
     * @return the {@link Builder} for further configuration
     * @since 0.2.2
     */
    public Builder tokenEndpointAuthenticationSigningAlgorithm(
        String authenticationSigningAlgorithm) {
      return claim(
     ...
     *
     * @param grantType the OAuth 2.0 {@code grant_type} that the Client will restrict itself to
     *     using
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * A {@code Consumer} of the OAuth 2.0 {@code grant_type} values that the Client will restrict
     * itself to using, allowing the ability to add, replace, or remove, OPTIONAL.
     *
     * @param grantTypesConsumer a {@code Consumer} of the OAuth 2.0 {@code grant_type} values that
     *     the Client will restrict itself to using
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * Add the OAuth 2.0 {@code response_type} that the Client will restrict itself to using,
     * OPTIONAL.
     *
     * @param responseType the OAuth 2.0 {@code response_type} that the Client will restrict itself
     *     to using
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * A {@code Consumer} of the OAuth 2.0 {@code response_type} values that the Client will
     * restrict itself to using, allowing the ability to add, replace, or remove, OPTIONAL.
     *
    ...
    /**
     * A {@code Consumer} of the OAuth 2.0 {@code scope} values that the Client will restrict itself
     * to using, allowing the ability to add, replace, or remove, OPTIONAL.
     *
     * @param scopesConsumer a {@code Consumer} of the OAuth 2.0 {@code scope} values that the
    ...
    /**
     * Sets the {@link SignatureAlgorithm JWS} algorithm required for signing the {@link OidcIdToken
     * ID Token} issued to the Client, OPTIONAL.
     *
     * @param idTokenSignedResponseAlgorithm the {@link SignatureAlgorithm JWS} algorithm required
     *     for signing the {@link OidcIdToken ID Token} issued to the Client
     * @return the {@link Builder} for further configuration
     */
    ...
    /**
     * Sets the Registration Access Token that can be used at the Client Configuration Endpoint,
     * OPTIONAL.
     *
     * @param registrationAccessToken the Registration Access Token that can be used at the Client
    ...
    /**
     * Sets the {@code URL} of the Client Configuration Endpoint where the Registration Access Token
     * can be used, OPTIONAL.
     *
     * @param registrationClientUrl the {@code URL} of the Client Configuration Endpoint where the
     *     Registration Access Token can be used
     * @return the {@link Builder} for further configuration
     * @since 0.2.1
     */
    ...
    /**
     * Provides access to every {@link #claim(String, Object)} declared so far allowing the ability
     ...
     *
     * <p>The following claims are REQUIRED: {@code client_id}, {@code redirect_uris}.
     *
    ...
    private void validate() {
      if (this.claims.get(OidcClientMetadataClaimNames.CLIENT_ID_ISSUED_AT) != null
        ...
        Assert.notNull(
            this.claims.get(OidcClientMetadataClaimNames.CLIENT_ID), "client_id cannot be null");
      }
      if (this.claims.get(OidcClientMetadataClaimNames.CLIENT_ID_ISSUED_AT) != null) {
        Assert.isInstanceOf(
            Instant.class,
            this.claims.get(OidcClientMetadataClaimNames.CLIENT_ID_ISSUED_AT),
            "client_id_issued_at must be of type Instant");
      }
      if (this.claims.get(OidcClientMetadataClaimNames.CLIENT_SECRET_EXPIRES_AT) != null) {
        Assert.notNull(
            this.claims.get(OidcClientMetadataClaimNames.CLIENT_SECRET),
      ...
      Assert.notNull(
          this.claims.get(OidcClientMetadataClaimNames.REDIRECT_URIS),
      ...
      Assert.notEmpty(
          (List<?>) this.claims.get(OidcClientMetadataClaimNames.REDIRECT_URIS),
          "redirect_uris cannot be empty");
      if (this.claims.get(OidcClientMetadataClaimNames.GRANT_TYPES) != null) {
        Assert.isInstanceOf(
            List.class,
            this.claims.get(OidcClientMetadataClaimNames.GRANT_TYPES),
            "grant_types must be of type List");
        Assert.notEmpty(
            (List<?>) this.claims.get(OidcClientMetadataClaimNames.GRANT_TYPES),
            "grant_types cannot be empty");
      }
      if (this.claims.get(OidcClientMetadataClaimNames.RESPONSE_TYPES) != null) {
        Assert.isInstanceOf(
            List.class,
            this.claims.get(OidcClientMetadataClaimNames.RESPONSE_TYPES),
            "response_types must be of type List");
        Assert.notEmpty(
            (List<?>) this.claims.get(OidcClientMetadataClaimNames.RESPONSE_TYPES),
            "response_types cannot be empty");
      }
      if (this.claims.get(OidcClientMetadataClaimNames.SCOPE) != null) {
        Assert.isInstanceOf(
            List.class,
            this.claims.get(OidcClientMetadataClaimNames.SCOPE),
            "scope must be of type List");
        Assert.notEmpty(
            (List<?>) this.claims.get(OidcClientMetadataClaimNames.SCOPE), "scope cannot be empty");
      }
      if (this.claims.get(OidcClientMetadataClaimNames.JWKS_URI) != null) {
        validateURL(
            this.claims.get(OidcClientMetadataClaimNames.JWKS_URI), "jwksUri must be a valid URL");
      }
    }
