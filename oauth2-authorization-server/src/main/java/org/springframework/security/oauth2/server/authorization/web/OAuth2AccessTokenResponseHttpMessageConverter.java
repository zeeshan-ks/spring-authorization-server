...
/**
 * TODO This class is a copy from Spring Security and should be removed after upgrading to Spring
 * Security 5.6.0 GA.
 *
 * <p>A {@link HttpMessageConverter} for an {@link OAuth2AccessTokenResponse OAuth 2.0 Access Token
 * Response}.
 *
 * @author Joe Grandja
 * @since 5.1
...
class OAuth2AccessTokenResponseHttpMessageConverter
  ...
  private Converter<Map<String, ?>, OAuth2AccessTokenResponse> accessTokenResponseConverter =
      new DefaultMapOAuth2AccessTokenResponseConverter();

  /** @deprecated This field should no longer be used */
  @Deprecated
  protected Converter<OAuth2AccessTokenResponse, Map<String, String>>
      tokenResponseParametersConverter = new OAuth2AccessTokenResponseMapConverter();

  private Converter<OAuth2AccessTokenResponse, Map<String, Object>>
  ...
  protected OAuth2AccessTokenResponse readInternal(
    ...
    try {
      ...
      if (this.tokenResponseConverter.getClass() != MapOAuth2AccessTokenResponseConverter.class) {
        ...
        tokenResponseParameters.forEach(
            (key, value) -> stringTokenResponseParameters.put(key, String.valueOf(value)));
  ...
  protected void writeInternal(
    ...
    try {
      Map<String, Object> tokenResponseParameters;
      // Only use deprecated converter if it has been set directly
      if (this.tokenResponseParametersConverter.getClass()
          != OAuth2AccessTokenResponseMapConverter.class) {
        tokenResponseParameters =
            new LinkedHashMap<>(this.tokenResponseParametersConverter.convert(tokenResponse));
      } else {
        tokenResponseParameters =
            this.accessTokenResponseParametersConverter.convert(tokenResponse);
      }
   ...
   * to an {@link OAuth2AccessTokenResponse}.
   *
   * @param accessTokenResponseConverter the {@link Converter} used for converting to an {@link
   *     OAuth2AccessTokenResponse}
   * @since 5.6
   */
   ...
   *
   * @deprecated Use {@link #setAccessTokenResponseParametersConverter(Converter)} instead
   ...
   * {@code Map} representation of the OAuth 2.0 Access Token Response parameters.
   *
   * @param accessTokenResponseParametersConverter the {@link Converter} used for converting to a
  ...
  public final void setAccessTokenResponseParametersConverter(
      Converter<OAuth2AccessTokenResponse, Map<String, Object>>
          accessTokenResponseParametersConverter) {
    Assert.notNull(
        accessTokenResponseParametersConverter,
        "accessTokenResponseParametersConverter cannot be null");
  ...
  static final class DefaultMapOAuth2AccessTokenResponseConverter
    ...
    private static OAuth2AccessToken.TokenType getAccessTokenType(
        Map<String, ?> tokenResponseParameters) {
    ...
    private static String getParameterValue(
        Map<String, ?> tokenResponseParameters, String parameterName) {
      Object obj = tokenResponseParameters.get(parameterName);
      return (obj != null) ? obj.toString() : null;
    }

    private static long getParameterValue(
        Map<String, ?> tokenResponseParameters, String parameterName, long defaultValue) {
